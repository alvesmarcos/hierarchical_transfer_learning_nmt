...
class CleanUp(Step):
    ...
    def __get_pairs(self, src_sentences, tgt_sentences):
        ...
        def _filter(p): return len(p[0].split()) < self.__max_length and len(
            p[1].split()) < self.__max_length
    ...
    def routine(self, _input):
        ...
        X_train, y_train = self.__get_pairs(
            _input['train'][0], _input['train'][1])
        logger.info("Valid set")
        X_valid, y_valid = self.__get_pairs(
            _input['valid'][0], _input['valid'][1])
